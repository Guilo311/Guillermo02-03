  ...
  fetch('https://api.anthropic.com/v1/messages', {
    ...
    .then(async (res) => {
      if (!res.ok) {
        ...
        opts.onError(err?.error?.message ?? `Erro HTTP ${res.status}`);
        opts.onDone();
        return;
      }
...
Plano ativo: ${activePlan ?? 'ESSENTIAL'} | Fonte: ${kpiSourceMode ?? 'fallback'}`;
    ...
    callClaudeStream({
      ...
      onError: (msg) => {
        setConversations(prev => prev.map(c =>
          c.id === activeConversation.id ? { ...c, messages: c.messages.map(m => m.id === assistantId ? { ...m, content: `⚠️ ${msg}` } : m) } : c
        ));
...
Responda sempre com base no contexto do projeto, seja objetivo e prático.`;
    ...
    callClaudeStream({
      ...
      onDone: () => setIsStreaming(false),
      onError: (msg) => {
        setProjects(prev => prev.map(p => p.id === proj.id ? { ...p, messages: p.messages.map(m => m.id === assistantId ? { ...m, content: `⚠️ ${msg}` } : m) } : p));
        setIsStreaming(false);
      },
    }).then(cancel => { abortClaudeRef.current = cancel; });
  };
                    ...
                    {activeConversation?.messages.map((message) => (
                      <div
                        ...
                        style={{
                          ...
                          fontSize: 13,
                          lineHeight: 1.6,
                          background: message.role === 'user' ? DARK.accent : DARK.surface,
